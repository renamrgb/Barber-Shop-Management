      ...
      <CCard class="mb-4">
        <CCardHeader>
          <CRow class="align-items-start">
            <CCol> <h4>Modelo de mensagem</h4> </CCol>
            <CCol id="teste-align">
              <router-link to="/forms/modeloMensagem/cadastro">
...
export default {
  ...
  methods: {
    ...
    alterar(id) {
      this.$router.push({ path: `/forms/modeloMensagem/cadastro/${id}` })
    },
    async getByTitle() {
      let data = await this.service.getByTitle(this.searchText);      
      this.itens = data;
    },
  },
